...
public class DownloadUtil {
    ...
    static  {
        //初始化list数据。
        trackersList.addAll(FileUtil.readLines(baseResourcePath+"aria2c/trackers.txt","UTF-8"));
        log.info("trackers==={}",trackersList);
    }

    public static void startDownload(String link,String trackers,String dist) throws IOException {
        StringBuffer sb = new StringBuffer();
        sb.append(DdnsStart.getAria2cInfPath());
        sb.append(" ");
        if(!StrUtil.isBlankOrUndefined(dist)){
            sb.append(" -o ").append(dist);
        }
        ...
        if(StrUtil.isBlankOrUndefined(link)){
            ...
            sb.append(" ").append(link).append(" <").append(link).append(".txt");
        }
        log.info("拼接好的命令为{}",sb);
